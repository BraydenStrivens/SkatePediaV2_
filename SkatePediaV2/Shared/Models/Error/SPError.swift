...
enum SPError: LocalizedError {
    ...
    case auth(AuthError)
    case videoUpload(VideoUploadError)
    case custom(String)
    case unknown
    ...
    var errorDescription: String? {
        ...
        case .videoUpload(let videoUploadError):
            return videoUploadError.errorDescription ?? "Something went wrong"
...
func mapToSPError(error: Error) -> SPError {
    ...
    if let videoUploadError = error as? VideoUploadError {
        print("MANUAL VIDEO UPLOAD ERROR")
        return .videoUpload(videoUploadError)
    }
