...
final class ChatMessagesViewModel: ObservableObject {
    ...
    func createUserChatDocuments(currentUser: User, withUserData: UserData) async {
        do {
            ...
            self.chatState = .failure(mapToSPError(error: error))
        }
    }
    ...
    func addListenerToFirstNMessages(sharedChatId: String) async {
        ...
        defer { listenerAdded = true }
        do {
            listener = try await MessagingManager.shared.sharedChatMessagesListenerQuery(
            ...
            errorStore.present(error, title: "Failed To Listen To Messages.")
        }
    }
    ...
    func fetchMoreMessage(sharedChatId: String) async {
        ...
        do {
            let (currentBatch, lastDocument) = try await MessagingManager.shared.fetchChatMessages(
                sharedChatId: sharedChatId,
                count: batchCount,
                lastDocument: lastDocument
            )
            
            messages.append(contentsOf: currentBatch)
            if let lastDocument { self.lastDocument = lastDocument }
            lastFetchSize = currentBatch.count
            
        } catch {
            errorStore.present(error, title: "Error Fetching Messages.")
        }
    }
    ...
    func sendMessage(userChat: UserChat, currentUser: User) async {
        ...
        do {
            ...
            errorStore.present(error, title: "Error Sending Message.")
        }
    }
    ...
    func hideMessage(sharedChatId: String, hiderId: String, messageId: String) async {
        do {
            ...
            errorStore.present(error, title: "Error Hiding Message.")
        }
    }
    ...
    func deleteMessage(sharedChatId: String, message: UserMessage) async {
        do {
            ...
            errorStore.present(error, title: "Error Deleting Message.")
        }
    }
    ...
    func loadImage(item: PhotosPickerItem) async {
        do {
            ...
            errorStore.present(error, title: "Error Loading Image")
        }
    }
    ...
    func loadVideo(item: PhotosPickerItem) async {
        do {
            ...
            errorStore.present(error, title: "Error Loading Video.")
        }
    }
