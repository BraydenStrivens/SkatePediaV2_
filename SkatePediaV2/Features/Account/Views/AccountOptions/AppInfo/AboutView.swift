...
/// A view that provides information about the app creator, SkatePedia, and how the app works.
///
/// This view is primarily informational and is organized into multiple sections:
/// 1. **About Me** – Personal introduction and contact info (email & Instagram).
/// 2. **About SkatePedia** – Context and background for the app.
/// 3. **Trick List** – Explanation of the trick list used in the app.
/// 4. **Trick Items** – Explanation of Trick Items and their purpose.
///
/// - Note: Contact buttons open the user's email app or Instagram app/web page if available.
struct AboutView: View {
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 20) {
                aboutViewSection(header: "About Me") { aboutMeSection }
                aboutViewSection(header: "About SkatePedia") { Text(aboutSkatePedia) }
                aboutViewSection(header: "Trick List") { Text(trickListExplanation) }
                aboutViewSection(header: "Trick Items") { Text(trickItemExplanation) }
            }
            .padding(12)
        }
        .customNavHeader(
        ...
        VStack {
            ...
            HStack {
                ...
                VStack(alignment: .trailing) {
                    ...
                    Button(email) { sendEmail(to: email) }
                    
                    let instaUsername = "b_bizzlemonizzle"
                    Button(instaUsername) { openInstagram(username: instaUsername) }
                }
    ...
    private func openInstagram(username: String) {
        ...
        openURL(UIApplication.shared.canOpenURL(appURL) ? appURL : webURL)
    }
    
    private func aboutViewSection<Content: View>(
        header: String,
