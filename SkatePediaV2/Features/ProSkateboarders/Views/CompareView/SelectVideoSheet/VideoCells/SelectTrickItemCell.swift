...
/// A selectable cell view for displaying a user's trick item. Used for selected a trick item video
/// for the comparison view.
///
/// Shows the user’s profile image, username, stance, and the trick item video.
...
struct SelectTrickItemCell: View {
    ...
    /// Header section displaying the user’s profile image, username, stance, and selection indicator.
        ...
        HStack(alignment: .top) {
            if let user = user {
                ...
                VStack(alignment: .leading) {
                    HStack {
                        CircularProfileImageView(photoUrl: user.profilePhoto?.photoUrl, size: .medium)
    ...
    /// Video player section that plays the trick item video.
    ///
